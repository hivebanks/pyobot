...
class Robot(object):
    ...
    def send_to_message(self):
        ...
        params = parse.urlencode({'port': self.port})
        data = request.urlopen(url + "/bot/get_qrcode.php?%s" % params).read()
        data_json = json.loads(data.decode("utf-8"))
        dict = data_json['rows']
        if int(dict['robot_alive']) != 1:
            # 获取机器人昵称，存入数据库
            params = parse.urlencode({'bot_name': self.bot.self.name, 'port': self.port})
            ...
            for group_name in all_groups:
                ...
                #插入数据库临时群表
                params = parse.urlencode({'group_name': group_name.name, 'port': self.port})
    ...
    def check_login(self):
        while True:
            ...
            params = parse.urlencode({'robot_alive': status, 'port':self.port})
               ...
               if name == g['name'] and int(g['is_del']) == 1:
                   ...
                   if int(g['is_flirt']) == 1:
                       if "@"+self.bot.self.name in msg.text:
                           ...
                           # 插入数据库
                           self.insert_message(msg.raw['CreateTime'], self.bot.self.name, ret, g['ba_id'], name, msg.raw['Type'], path2)
    ...
    def sel_frind_message(self):
        ...
        def sel_friend(msg):
            ...
            # 插入数据库
            self.insert_message(msg.raw['CreateTime'], self.bot.self.name, ret, 'friend', 'friend', msg.raw['Type'], '')
